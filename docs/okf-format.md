...
title: What pr-policy is # required here
...
tags: [pr-policy, checks] # optional
---
```
...
- **`resource`** — a repo-relative path that must **exist on disk** — is
  **required on every non-`Design`, non-`Reference` page**, binding each doc to
  ...
  an intent or a convention, not one source file). Each policy check gets a
  `Library` page under `docs/checks/` whose `resource` is its module in
  `src/checks/`; the pages about the suite as a whole are `Reference` or `Design`.
